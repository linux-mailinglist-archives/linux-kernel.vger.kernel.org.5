...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 698647FB15C
	for <lists+linux-kernel@lfdr.de>; Tue, 28 Nov 2023 06:39:16 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1343553AbjK1FjH (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 28 Nov 2023 00:39:07 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:42684 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S232615AbjK1FjF (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 28 Nov 2023 00:39:05 -0500
Received: from mgamail.intel.com (mgamail.intel.com [134.134.136.31])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 31A29C4
        for <linux-kernel@vger.kernel.org>; Mon, 27 Nov 2023 21:39:11 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple;
  d=intel.com; i=@intel.com; q=dns/txt; s=Intel;
  t=1701149951; x=1732685951;
  h=from:to:cc:subject:in-reply-to:references:date:
   message-id:mime-version:content-transfer-encoding;
  bh=kSB8jAGzkfz946UtyNz1cb0xEsgJtUvO9yZaeDIA+C8=;
  b=l7f+b0wyFIlcJS1AZ7Wb80RatbQ3HquLp5dMRePtY37useJppHBRuM6M
   vFaUK9+r21MAbkuupwBdTWk1Dtkiv5waiqGzBaZjSBQo3jTvuDpIcyfaz
   WJIaI5YbHE1RBPrtq4GtfuDTLTHZ+VKBsAv0DHp5+i6DmXPxifI/Nf+O1
   +l/DQaNoylwCFP5hgKPn4qEcy1RgnNaAbRoVHlhPBHllkvQ/Ep9LO1fRr
   Ykd1JFlW2tLXfUlTIU9LTmQTz2rRYdB813iqFYFspzRetkI8WkOQM/0F1
   tiXAZb3PVl1G3lNflJRWCd2U0f2xHMRCNg0t3tn/0q7al0LOiwvR1XsVT
   Q==;
X-IronPort-AV: E=McAfee;i="6600,9927,10907"; a="457184903"
X-IronPort-AV: E=Sophos;i="6.04,233,1695711600"; 
   d="scan'208";a="457184903"
Received: from orviesa002.jf.intel.com ([10.64.159.142])
  by orsmga104.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 27 Nov 2023 21:39:09 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="6.04,233,1695711600"; 
   d="scan'208";a="9837544"
Received: from yhuang6-desk2.sh.intel.com (HELO yhuang6-desk2.ccr.corp.intel.com) ([10.238.208.55])
  by orviesa002-auth.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 27 Nov 2023 21:39:07 -0800
...
Subject: Re: [PATCH v10] mm: vmscan: try to reclaim swapcache pages if no
 swap space
In-Reply-To: <CAJD7tkZKS=1BSoaCnF5wkto5z3-_sx=6GSVqdhXz=Dcw_-C3BA@mail.gmail.com>
        (Yosry Ahmed's message of "Mon, 27 Nov 2023 20:13:38 -0800")
...
Date:   Tue, 28 Nov 2023 13:37:05 +0800
Message-ID: <87bkbe5tha.fsf@yhuang6-desk2.ccr.corp.intel.com>
...
>> Yosry Ahmed <yosryahmed@google.com> writes:
>>
>> > On Mon, Nov 27, 2023 at 7:21=E2=80=AFPM Huang, Ying <ying.huang@intel.=
com> wrote:
>> >>
>> >> Yosry Ahmed <yosryahmed@google.com> writes:
>> >>
...
@intel.com> wrote:
>> >> >> > > >  I agree with Ying that anonymous pages typically have diffe=
rent page
>> >> >> > > > access patterns than file pages, so we might want to treat t=
hem
>> >> >> > > > differently to reclaim them effectively.
>> >> >> > > > One random idea:
>> >> >> > > > How about we put the anonymous page in a swap cache in a dif=
ferent LRU
>> >> >> > > > than the rest of the anonymous pages. Then shrinking against=
 those
>> >> >> > > > pages in the swap cache would be more effective.Instead of h=
aving
>> >> >> > > > [anon, file] LRU, now we have [anon not in swap cache, anon =
in swap
>> >> >> > > > cache, file] LRU
>> >> >> > >
>> >> >> > > I don't think that it is necessary.  The patch is only for a s=
pecial use
>> >> >> > > case.  Where the swap device is used up while some pages are i=
n swap
>> >> >> > > cache.  The patch will kill performance, but it is used to avo=
id OOM
>> >> >> > > only, not to improve performance.  Per my understanding, we wi=
ll not use
...
>> >> >> > author yet. That is one important aspect to evaluate how valuabl=
e is
>> >> >> > that patch.
>> >> >>
>> >> >> Exactly. Since swap cache has different life time with page cache,=
 they
>> >> >> would be usually dropped when pages are unmapped(unless they are s=
hared
...
>> >> > avoidance will come at the cost of extra work in reclaim to swap th=
ose
>> >> > pages out.
>> >> >
>> >> > The only case where I think this might be harmful is if there's ple=
nty
>> >> > of pages to reclaim on the file LRU, and instead we opt to chase do=
wn
...
>> >> > than the number of pages on the file LRU or similar? Just make sure=
 we
>> >> > don't chase the swapcache pages down if there's plenty to scan on t=
he
>> >> > file LRU?
>> >>
...
>> >>   not reclaimed yet.
>> >>
>> >> - group 2: pages have been written out, but were failed to be reclaim=
ed
...
>> >>
>> >> The main target of the original patch should be group 1.  And the pag=
es
...
>> >> And we may need to reclaim the swap entries instead of pages if the p=
ages

...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 127A3810821
	for <lists+linux-kernel@lfdr.de>; Wed, 13 Dec 2023 03:19:46 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1378226AbjLMCTg (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 12 Dec 2023 21:19:36 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:54380 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1378314AbjLMCTW (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 12 Dec 2023 21:19:22 -0500
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id C93D2100
        for <linux-kernel@vger.kernel.org>; Tue, 12 Dec 2023 18:19:27 -0800 (PST)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id D9FD8C433C8;
        Wed, 13 Dec 2023 02:19:25 +0000 (UTC)
Received: from rostedt by gandalf with local (Exim 4.97)
        (envelope-from <rostedt@goodmis.org>)
        id 1rDEr3-00000002TnI-2m7g;
        Tue, 12 Dec 2023 21:20:09 -0500
Message-ID: <20231213022009.441698612@goodmis.org>
User-Agent: quilt/0.67
Date:   Tue, 12 Dec 2023 21:19:19 -0500
...
Subject: [PATCH v2 05/15] ring-buffer: Read and write to ring buffers with custom sub buffer
 size
...
@@ -5684,8 +5698,12 @@ void ring_buffer_free_read_page(struct trace_buffer *buffer, int cpu, void *data
...
+	if (page_ref_count(page) > 1 || data_page->order != buffer->subbuf_order)
 	...
 	local_irq_save(flags);
@@ -5700,7 +5718,8 @@ void ring_buffer_free_read_page(struct trace_buffer *buffer, int cpu, void *data
 	local_irq_restore(flags);
...
@@ -5721,9 +5740,10 @@ EXPORT_SYMBOL_GPL(ring_buffer_free_read_page);
...
@@ -5738,7 +5758,8 @@ EXPORT_SYMBOL_GPL(ring_buffer_free_read_page);
...
@@ -5763,10 +5784,12 @@ int ring_buffer_read_page(struct trace_buffer *buffer,
...
@@ -5860,11 +5883,11 @@ int ring_buffer_read_page(struct trace_buffer *buffer,
 		/* swap the pages */
 		rb_init_page(bpage);
 		bpage = reader->page;
...
@@ -5909,6 +5932,18 @@ int ring_buffer_read_page(struct trace_buffer *buffer,
 }
...
index 4ac9da316f68..bb43605aacf6 100644
--- a/kernel/trace/trace.c
+++ b/kernel/trace/trace.c
@@ -8279,6 +8279,8 @@ tracing_buffers_read(struct file *filp, char __user *ubuf,
 {
...
@@ -8290,6 +8292,8 @@ tracing_buffers_read(struct file *filp, char __user *ubuf,
 		return -EBUSY;
 #endif
...
+
 	if (!info->spare) {
 		info->spare = ring_buffer_alloc_read_page(iter->array_buffer->buffer,
 							  iter->cpu_file);
@@ -8304,13 +8308,13 @@ tracing_buffers_read(struct file *filp, char __user *ubuf,
 		return ret;
...
@@ -8331,11 +8335,11 @@ tracing_buffers_read(struct file *filp, char __user *ubuf,
...
@@ -8446,6 +8450,7 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
...
@@ -8454,13 +8459,14 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
 		return -EBUSY;
 #endif
...
@@ -8470,7 +8476,7 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
 	trace_access_lock(iter->cpu_file);
...
@@ -8491,7 +8497,7 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
 		}
...
@@ -8500,14 +8506,14 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
 			break;
 		}

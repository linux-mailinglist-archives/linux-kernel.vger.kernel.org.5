Return-Path: <linux-kernel+bounces-49114-lists+linux-kernel=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-kernel@lfdr.de
Delivered-To: lists+linux-kernel@lfdr.de
Received: from ny.mirrors.kernel.org (ny.mirrors.kernel.org [IPv6:2604:1380:45d1:ec00::1])
	by mail.lfdr.de (Postfix) with ESMTPS id 86186846600
	for <lists+linux-kernel@lfdr.de>; Fri,  2 Feb 2024 03:48:49 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by ny.mirrors.kernel.org (Postfix) with ESMTPS id 9CB3B1C241E1
	for <lists+linux-kernel@lfdr.de>; Fri,  2 Feb 2024 02:48:48 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 92A54C130;
	Fri,  2 Feb 2024 02:48:46 +0000 (UTC)
Received: from mail-pg1-f170.google.com (mail-pg1-f170.google.com [209.85.215.170])
	(using TLSv1.2 with cipher ECDHE-RSA-AES128-GCM-SHA256 (128/128 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 491BCD30E;
	Fri,  2 Feb 2024 02:48:40 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org; arc=none smtp.client-ip=209.85.215.170
ARC-Seal:i=1; a=rsa-sha256; d=subspace.kernel.org; s=arc-20240116;
	t=1706842123; cv=none; b=iDskkbUyGN7vmi3X+Rct+Bp89i3gFcFQilFM6aUP0Qnw3qReAAfCKFV6TS3r8rMgEP2BCn+E98xJ/jsXRVwHAF0BzVyFaGkMX65EcQ/0QsjGKos+J/OSgd+StPmoBFVy4IqZJEUbmZLreuUSRG7BHOTBZ6XwxhtXcbfzFqvNLQc=
ARC-Message-Signature:i=1; a=rsa-sha256; d=subspace.kernel.org;
	s=arc-20240116; t=1706842123; c=relaxed/simple;
	bh=cXaNoUXPhvzgRzTTHB7yaesRzRjYB8kbKspopsNVcsM=;
	h=MIME-Version:References:In-Reply-To:From:Date:Message-ID:Subject:
	 To:Cc:Content-Type; b=XVULoPIlNzJskJaAjwo/WHAiMF5H0kY0txp3hPdl9EnrbPC/PSADdT9PevoPIgzjFQEJBzdhZD1nDRYrsRs7y6alqx+Mre/LFkOAZ/Ywaa4jwW3jdmAbpOBuByYNr5ttJxYCWPkNbEZaaZgy6xbSRhqK8ybRkYd2swwHCYKelsw=
...
Received: by mail-pg1-f170.google.com with SMTP id 41be03b00d2f7-5cf2d73a183so2061612a12.1;
        Thu, 01 Feb 2024 18:48:40 -0800 (PST)
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=1e100.net; s=20230601; t=1706842119; x=1707446919;
        h=content-transfer-encoding:cc:to:subject:message-id:date:from
         :in-reply-to:references:mime-version:x-gm-message-state:from:to:cc
         :subject:date:message-id:reply-to;
        bh=4jRQiLXFKy//0l8izSu1B3NaWxeLS4veuF/+CakN3Vo=;
        b=
X-Gm-Message-State: AOJu0YzTNi9+abgBvJI/ToP34gL3t5cMkT5hdyKMhJzkojr4aXNVX6My
	P1Zt8mi/IcpkarlYt5n9fxVit6T9s8k+PeoPKYNI8g61w5/dMv8k4adY/Pegq/H6G7bvBQsfpI7
	34MtJvLDHRrDoTfkpk+H5g+o+05o=
X-Google-Smtp-Source: AGHT+IGpST5jAutGOfVpTTaltWwxx/HwZ9xLSa0qSng8NHOjBUrYQT/SA7rR/VKhHzxbxDEjciZ3SJygO14tDyG6JXk=
X-Received: by 2002:a17:90a:d585:b0:296:930:8792 with SMTP id
 v5-20020a17090ad58500b0029609308792mr1497173pju.9.1706842118785; Thu, 01 Feb
 2024 18:48:38 -0800 (PST)
Precedence: bulk
...
In-Reply-To: <20240103050635.391888-2-irogers@google.com>
From: Namhyung Kim <namhyung@kernel.org>
Date: Thu, 1 Feb 2024 18:48:27 -0800
Message-ID: <CAM9d7chUnXOFXsz-gdK4V=RX4DfJMdBOzfzso_uKSj=00xNeZg@mail.gmail.com>
...
On Tue, Jan 2, 2024 at 9:07=E2=80=AFPM Ian Rogers <irogers@google.com> wrot=
e:
>
...
> @@ -156,6 +156,9 @@ static int test__maps__merge_in(struct test_suite *t =
...
> @@ -168,6 +168,7 @@ struct map *map__new(struct machine *machine, u64 sta=
rt, u64 len,
>                 if (dso =3D=3D NULL)
>                         goto out_delete;
>
...
> -       for (map =3D maps__first(maps); map; map =3D map_rb_node__next(ma=
p))
> +static void check_invariants(const struct maps *maps __maybe_unused)
> +{
> +#ifndef NDEBUG
> +       assert(RC_CHK_ACCESS(maps)->nr_maps <=3D RC_CHK_ACCESS(maps)->nr_=
maps_allocated);
> +       for (unsigned int i =3D 0; i < RC_CHK_ACCESS(maps)->nr_maps; i++)=
 {
> +               struct map *map =3D RC_CHK_ACCESS(maps)->maps_by_address[=
i];
> +
> +               /* Check map is well-formed. */
> +               assert(map__end(map) =3D=3D 0 || map__start(map) <=3D map=
__end(map));
...
> +
> +               if (i > 0) {
> +                       struct map *prev =3D RC_CHK_ACCESS(maps)->maps_by=
_address[i - 1];
> +
 ...
 */
> +                               assert(map__start(prev) <=3D map__start(m=
ap));
> +                               /*
> +                                * If the ends of maps aren't broken (dur=
ing
> +                                * construction) then they should be orde=
red
> +                                * too.
> +                                */
> +                               if (!RC_CHK_ACCESS(maps)->ends_broken) {
> +                                       assert(map__end(prev) <=3D map__e=
nd(map));
> +                                       assert(map__end(prev) <=3D map__s=
tart(map) ||
> +                                              map__start(prev) =3D=3D ma=
p__start(map));
> +                               }
> +                       }
> +               }
...
> +               for (unsigned int i =3D 0; i < RC_CHK_ACCESS(maps)->nr_ma=
ps; i++) {
> +                       struct map *map =3D RC_CHK_ACCESS(maps)->maps_by_=
name[i];
>
...
> +                       /*
> +                        * Maps by name maps should be in maps_by_address=
, so
...
> +static void maps__set_maps_by_address(struct maps *maps, struct map **ne=
w)
>  {
...
> -       if (!next)
> -               return NULL;
> +static void maps__set_maps_by_address_sorted(struct maps *maps, bool val=
ue)
> +{
...
>  }
>
> -static struct map_rb_node *maps__find_node(struct maps *maps, struct map=
 *map)
...
> -               if (rb_node->RC_CHK_ACCESS(map) =3D=3D RC_CHK_ACCESS(map)=
)
> -                       return rb_node;
> -       }
> -       return NULL;
...
> +{
>         /*
> -        * If we already performed some search by name, then we need to a=
dd the just
...
> -               if (maps__nr_maps(maps) > RC_CHK_ACCESS(maps)->nr_maps_al=
located) {
> -                       int nr_allocate =3D maps__nr_maps(maps) * 2;
> -                       struct map **maps_by_name =3D realloc(maps__maps_=
by_name(maps),
> -                                                           nr_allocate *=
 sizeof(map));
> +       for (unsigned int i =3D 0; i < maps__nr_maps(maps); i++)
...
> +}
>
> -                       RC_CHK_ACCESS(maps)->maps_by_name =3D maps_by_nam=
e;
> -                       RC_CHK_ACCESS(maps)->nr_maps_allocated =3D nr_all=
ocate;
...
> +                       /* Ensure maps with the same addresses have a fix=
ed order. */
> +                       if (RC_CHK_ACCESS(map_a) =3D=3D RC_CHK_ACCESS(map=
_b))
> +                               return 0;
> +                       return (intptr_t)RC_CHK_ACCESS(map_a) > (intptr_t=
)RC_CHK_ACCESS(map_b)
> +                               ? 1 : -1;
>                 }
> -               maps__maps_by_name(maps)[maps__nr_maps(maps) - 1] =3D map=
__get(map);
...
>  }
>
> -static void __maps__remove(struct maps *maps, struct map_rb_node *rb_nod=
e)
...
> -               map__put(pos->map);
> -               free(pos);
> +       if (ret =3D=3D 0 && RC_CHK_ACCESS(map_a) !=3D RC_CHK_ACCESS(map_b=
)) {
...
> +
> +               if (!maps_by_name) {
> +                       maps_by_name =3D malloc(RC_CHK_ACCESS(maps)->nr_m=
aps_allocated *
> +                                       sizeof(*maps_by_name));
> +                       if (!maps_by_name)
> +                               err =3D -ENOMEM;
...
> +                                       maps_by_name[i] =3D map__get(maps=
_by_address[i]);
> +                       }
> +               }
> +               if (!err) {
...
> +static unsigned int maps__by_address_index(const struct maps *maps, cons=
t struct map *map)
>  {
...
> +       if (maps__maps_by_address_sorted(maps)) {
> +               struct map **mapp =3D
> +                       bsearch(&map, maps__maps_by_address(maps), maps__=
nr_maps(maps),
...
> +               for (unsigned int i =3D 0; i < maps__nr_maps(maps); i++) =
{
> +                       if (RC_CHK_ACCESS(maps_by_address[i]) =3D=3D RC_C=
HK_ACCESS(map))
> +                               return i;
> +               }
> +       }
...
>
> -struct maps *maps__new(struct machine *machine)
> +static unsigned int maps__by_name_index(const struct maps *maps, const s=
truct map *map)
>  {
> -       struct maps *result;
> -       RC_STRUCT(maps) *maps =3D zalloc(sizeof(*maps));
...
> +               for (unsigned int i =3D 0; i < maps__nr_maps(maps); i++) =
{
> +                       if (RC_CHK_ACCESS(maps_by_name[i]) =3D=3D RC_CHK_=
ACCESS(map))
> +                               return i;
> +               }
> +       }
...
> +
> +               maps_by_address =3D realloc(maps_by_address, nr_allocate =
* sizeof(new));
> +               if (!maps_by_address)
> +                       return -ENOMEM;
> +
...
> +               if (maps_by_name) {
> +                       maps_by_name =3D realloc(maps_by_name, nr_allocat=
e * sizeof(new));
> +                       if (!maps_by_name) {
> +                               /*
> +                                * If by name fails, just disable by name=
 and it will
...
> +
> +       /*
> +        * Recompute if things are sorted. If things are inserted in a so=
rted
...
> +               maps__set_maps_by_address_sorted(maps, true);
> +               maps__set_maps_by_name_sorted(maps, maps_by_name !=3D NUL=
L);
> +       } else {
> +               /* Sorted if maps were already sorted and this map starts=
 after the last one. */
> +               maps__set_maps_by_address_sorted(maps,
> +                       maps__maps_by_address_sorted(maps) &&
> +                       map__end(maps_by_address[nr_maps - 2]) <=3D map__=
start(new));
...
>  }
>
>  void maps__remove_maps(struct maps *maps, bool (*cb)(struct map *map, vo=
id *data), void *data)
>  {
> -       struct map_rb_node *pos, *next;
...
> +                       i++;
>         }
> -       if (maps__maps_by_name(maps) && start_nr_maps !=3D maps__nr_maps(=
maps))
> -               __maps__free_maps_by_name(maps);
> -
>         up_write(maps__lock(maps));
>  }
>
> @@ -300,7 +491,7 @@ struct symbol *maps__find_symbol(struct maps *maps, u=
64 addr, struct map **mapp)
>         /* Ensure map is loaded before using map->map_ip */
>         if (map !=3D NULL && map__load(map) >=3D 0) {
>                 if (mapp !=3D NULL)
> -                       *mapp =3D map;
> +                       *mapp =3D map; // TODO: map_put on else path when=
 find returns a get.
>                 return map__find_symbol(map, map__map_ip(map, addr));
>         }
>
> @@ -348,7 +539,7 @@ int maps__find_ams(struct maps *maps, struct addr_map=
_symbol *ams)
>         if (ams->addr < map__start(ams->ms.map) || ams->addr >=3D map__en=
d(ams->ms.map)) {
>                 if (maps =3D=3D NULL)
>                         return -1;
> -               ams->ms.map =3D maps__find(maps, ams->addr);
> +               ams->ms.map =3D maps__find(maps, ams->addr);  // TODO: ma=
p_get
...
>   */
> -static struct rb_node *first_ending_after(struct maps *maps, const struc=
t map *map)
> +static unsigned int first_ending_after(struct maps *maps, const struct m=
ap *map)
>  {
> -       struct rb_root *root;
> -       struct rb_node *next, *first;
> +       struct map **maps_by_address =3D maps__maps_by_address(maps);
> +       int low =3D 0, high =3D (int)maps__nr_maps(maps) - 1, first =3D h=
igh + 1;
> +
> +       assert(maps__maps_by_address_sorted(maps));
> +       if (low <=3D high && map__end(maps_by_address[0]) > map__start(ma=
p))
> +               return 0;
>
...
> -       first =3D NULL;
> -       while (next) {
> -               struct map_rb_node *pos =3D rb_entry(next, struct map_rb_=
node, rb_node);
> +       while (low <=3D high) {
...
>         return first;
>  }
> @@ -419,171 +614,249 @@ static struct rb_node *first_ending_after(struct =
maps *maps, const struct map *m
>   * Adds new to maps, if new overlaps existing entries then the existing =
maps are
>   * adjusted or removed so that new fits without overlapping any entries.
>   */
> -int maps__fixup_overlap_and_insert(struct maps *maps, struct map *new)
> +static int __maps__fixup_overlap_and_insert(struct maps *maps, struct ma=
p *new)
>  {
> -
> -       struct rb_node *next;
...
> +        */
> +       for (unsigned int i =3D first_ending_after(maps, new); i < maps__=
nr_maps(maps); ) {
...
>                  * Maps are ordered by start: next will not overlap for s=
ure.
>                  */
...
> -
> -                       if (use_browser) {
> -                               pr_debug("overlapping maps in %s (disable=
 tui for more info)\n",
> -                                        map__dso(new)->name);
> -                       } else {
...
> -                       }
> +               if (use_browser) {
> +                       pr_debug("overlapping maps in %s (disable tui for=
 more info)\n",
> +                               map__dso(new)->name);
...
> +                       /* Map starts within existing map. Need to shorte=
n the existing map. */
...
> -                       map__add_pgoff(after, map__end(new) - map__start(=
pos->map));
...
> -               free(pos);
> +               /*
> +                * If adding one entry, for `before` or `after`, we can r=
eplace
> +                * the existing entry. If both `before` and `after` are
> +                * necessary than an insert is needed. If the existing en=
try
> +                * entirely overlaps the existing entry it can just be re=
moved.
> +                */
> +               if (before) {
...
{
> +                                       /*
> +                                        * Sorting broken so invariants d=
on't
...
> +                               }
> +                               /*
> +                                * Maps are still ordered, skip after and=
 go to
...
> +                       __maps__remove(maps, pos);
> +                       /*
> +                        * Maps are ordered but no need to increase `i` a=
s the
...
> +       if (maps__empty(dest)) {
> +               /* No existing mappings so just copy from parent to avoid=
 reallocs in insert. */
...
> +                                       malloc(nr_maps_allocated * sizeof=
(struct map *));
> +                       }
> +
> +                       RC_CHK_ACCESS(dest)->maps_by_address =3D dest_map=
s_by_address;
> +                       RC_CHK_ACCESS(dest)->maps_by_name =3D dest_maps_b=
y_name;
> +                       RC_CHK_ACCESS(dest)->nr_maps_allocated =3D nr_map=
s_allocated;
>                 }
>
...
> +                               map__put(new);
> +               }
> +               maps__set_maps_by_address_sorted(dest, maps__maps_by_addr=
ess_sorted(parent));
> +               if (!err) {
> +                       RC_CHK_ACCESS(dest)->last_search_by_name_idx =3D
> +                               RC_CHK_ACCESS(parent)->last_search_by_nam=
e_idx;
> +                       maps__set_maps_by_name_sorted(dest,
> +                                               dest_maps_by_name &&
> +                                               maps__maps_by_name_sorted=
(parent));
> +               } else {
> +                       RC_CHK_ACCESS(dest)->last_search_by_name_idx =3D =
0;
...
> -               /*
> -                * Ensure distinct but name equal maps have an order in p=
art to
...
> @@ -593,126 +866,113 @@ static int map__strcmp_name(const void *name, con=
st void *b)
...
> -{
> -       qsort(maps__maps_by_name(maps), maps__nr_maps(maps), sizeof(struc=
t map *), map__strcmp);
> -}
> -
...
> -}
> -
> -static struct map *__maps__find_by_name(struct maps *maps, const char *n=
ame)
...
> +               unsigned int i;
>
> -       mapp =3D bsearch(name, maps__maps_by_name(maps), maps__nr_maps(ma=
ps),
...
{
> +                       struct dso *dso =3D map__dso(maps__maps_by_name(m=
aps)[i]);
>
> +                       if (dso && strcmp(dso->short_name, name) =3D=3D 0=
) {
...
> -               const struct dso *dso =3D map__dso(RC_CHK_ACCESS(maps)->l=
ast_search_by_name);
> +               /* Second search sorted array. */
> +               if (!done && maps__maps_by_name_sorted(maps)) {
> +                       struct map **mapp =3D
> +                               bsearch(name, maps__maps_by_name(maps), m=
aps__nr_maps(maps),
> +                                       sizeof(*mapp), map__strcmp_name);
>
> -               if (strcmp(dso->short_name, name) =3D=3D 0) {
> -                       map =3D RC_CHK_ACCESS(maps)->last_search_by_name;
> -                       goto out_unlock;
> +                       if (mapp) {
> +                               result =3D *mapp; // TODO: map__get
> +                               i =3D mapp - maps__maps_by_name(maps);
> +                               RC_CHK_ACCESS(maps)->last_search_by_name_=
idx =3D i;
> +                       }
> +                       done =3D true;
>                 }
> -       }
> -       /*
> -        * If we have maps->maps_by_name, then the name isn't in the rbtr=
ee,
> -        * as maps->maps_by_name mirrors the rbtree when lookups by name =
are
> -        * made.
> -        */
...
> -               dso =3D map__dso(map);
> -               if (strcmp(dso->short_name, name) =3D=3D 0) {
> -                       RC_CHK_ACCESS(maps)->last_search_by_name =3D map;
> -                       goto out_unlock;
> +               up_read(maps__lock(maps));
> +               if (!done) {
...
> +                               /*
> +                                * Memory allocation failed do linear sea=
rch
...
> +
> +                               down_read(maps__lock(maps));
> +                               maps_by_address =3D  maps__maps_by_addres=
s(maps);
> +                               n =3D maps__nr_maps(maps);
> +                               for (i =3D 0; i < n; i++) {
> +                                       struct map *pos =3D maps_by_addre=
ss[i];
> +                                       struct dso *dso =3D map__dso(pos)=
;
> +
> +                                       if (dso && strcmp(dso->short_name=
, name) =3D=3D 0) {
> +                                               result =3D pos; // TODO: =
map__get
> +                                               break;
...
> +               result =3D maps__maps_by_address(maps)[i]; // TODO: map__=
get
>
> -       return NULL;
> +       up_read(maps__lock(maps));
...
: NULL;
> -       overlaps =3D rb_node && map__start(rb_node->map) < map__end(new_m=
ap);
> -       up_read(maps__lock(kmaps));
> +               up_read(maps__lock(kmaps));
> +
> +               /* First after binary search requires sorted maps. Sort a=
nd try again. */
...
> +       /* Plain insert with a read-lock failed, try again now with the w=
rite lock. */
...
> -                                *       |old....|    ->       |old....|
> -                                */
> -                               struct map_list_node *m =3D map_list_node=
__new();
...
*/
> +       merged_nr_maps_allocated =3D RC_CHK_ACCESS(kmaps)->nr_maps_alloca=
ted;
> +       if (kmaps__nr_maps + 1 =3D=3D merged_nr_maps_allocated)
> +               merged_nr_maps_allocated++;
> +
> +       merged_maps_by_address =3D malloc(merged_nr_maps_allocated * size=
of(*merged_maps_by_address));
> +       if (!merged_maps_by_address) {
> +               up_write(maps__lock(kmaps));
> +               return -ENOMEM;
> +       }
> +       RC_CHK_ACCESS(kmaps)->maps_by_address =3D merged_maps_by_address;
...
> +       RC_CHK_ACCESS(kmaps)->nr_maps_allocated =3D merged_nr_maps_alloca=
ted;
...
> +               merged_maps_by_address[i] =3D map__get(kmaps_maps_by_addr=
ess[i]);
>
...
> -                               map__add_pgoff(new_map, map__end(old_map)=
 - map__start(new_map));
...
> -               }
> -       }
> +       /* Add the new map, it will be split when the later overlapping m=
appings are added. */
...
> +       /* Insert mappings after new_map, splitting new_map in the proces=
s. */
> +       for (unsigned int i =3D first_after_; i < kmaps__nr_maps; i++)
> +               __maps__fixup_overlap_and_insert(kmaps, kmaps_maps_by_add=
ress[i]);
>
...
> @@ -25,21 +25,56 @@ static inline struct map_list_node *map_list_node__ne=
w(void)
...
> + *
> + * Sorting is done with the write lock, iteration and binary searching h=
appens
> + * under the read lock requiring being sorted. There is a race between s=
orting
> + * releasing the write lock and acquiring the read lock for iteration/se=
arching
> + * where another thread could insert and break the sorting of the maps. =
In
> + * practice inserting maps should be rare meaning that the race shouldn'=
t lead
...
> +       /**
> +        * @maps_by_address: array of maps sorted by their starting addre=
ss if
> +        * maps_by_address_sorted is true.
> +        */
> +       struct map       **maps_by_address;
> +       /**
> +        * @maps_by_name: optional array of maps sorted by their dso name=
 if
...
> +       unsigned int     nr_maps;
> +       /**
> +        * @nr_maps_allocated: number of entries in maps_by_address and p=
ossibly
> +        * maps_by_name.
> +        */
> +       unsigned int     nr_maps_allocated;
> +       /**
> +        * @last_search_by_name_idx: cache of last found by name entry's =
index
...
>
> +struct map *maps__find(struct maps *maps, u64 addr);
>  struct symbol *maps__find_symbol(struct maps *maps, u64 addr, struct map=
 **mapp);

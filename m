...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id CAE6277CE11
	for <lists+linux-kernel@lfdr.de>; Tue, 15 Aug 2023 16:29:12 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S237625AbjHOO2l (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 15 Aug 2023 10:28:41 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:37282 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S237618AbjHOO2K (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 15 Aug 2023 10:28:10 -0400
Received: from dfw.source.kernel.org (dfw.source.kernel.org [IPv6:2604:1380:4641:c500::1])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 77E66E7A;
        Tue, 15 Aug 2023 07:28:06 -0700 (PDT)
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        (using TLSv1.3 with cipher TLS_AES_256_GCM_SHA384 (256/256 bits)
         key-exchange X25519 server-signature RSA-PSS (2048 bits))
        (No client certificate requested)
        by dfw.source.kernel.org (Postfix) with ESMTPS id D05BE65276;
        Tue, 15 Aug 2023 14:28:05 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 33308C433C8;
        Tue, 15 Aug 2023 14:28:05 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1692109685;
        bh=zmwNO1kOn6BhiKwwHmdZxyx2aGHzZD4oSJdhZ3RYdS0=;
        h=Date:From:To:Cc:Subject:Reply-To:References:In-Reply-To:From;
        b=
Received: by paulmck-ThinkPad-P17-Gen-1.home (Postfix, from userid 1000)
        id BF80FCE0455; Tue, 15 Aug 2023 07:28:04 -0700 (PDT)
Date:   Tue, 15 Aug 2023 07:28:04 -0700
From:   "Paul E. McKenney" <paulmck@kernel.org>
To:     Patrick Yingxi Pan <pyxchina92929@gmail.com>
Cc:     Gang Li <gang.li@linux.dev>, Alex Shi <alex.shi@linux.alibaba.com>,
...
Subject: Re: [PATCH RESEND v1] docs/zh_CN: add zh_CN translation for
 memory-barriers.txt
Message-ID: <8bf246a1-0779-48f3-9653-30c04a5b466a@paulmck-laptop>
...
In-Reply-To: <CANJ3EgGNQhGdppzYWxLOoJ9kqdCVxOOCi89-6qq638fyqnm2fA@mail.gmail.com>
...
On Tue, Aug 15, 2023 at 07:08:16PM +0800, Patrick Yingxi Pan wrote:
...
You both are completely within your rights, in fact are encouraged, to
submit patches to the English-language version that improve the wording.
Future maintenance problem solved.  ;-)
...
> 
> Let me re-translate some paragraphs back to English for comparison to
> give every one a general impression.
> 
> > CPU可以通过一些本地缓存来提高指令运行效率，内存操作会被缓存在当前CPU上，
> > 每个 CPU 都能按顺序看到自己的内存操作。但每个缓存项写入到主存的顺序未知，
> > 在上图中，即每条指令的结果穿过虚线的顺序未知。
> 
...
> 
> The re-translation of the Chinese version:
> 
> CPU can improve instruction execution efficiency with some local caches.
> Memory operations will be cached locally on current CPU.
> Each CPU can see their own memory operations in-order.
> But the order in which the cache entries are sent to the main memory
> is uncertain. In the above diagram, the order in which operations
> cross the dotted lines is uncertain.
> 
> > CPU 外部可能看到如下任意顺序
> 
> The original English version:
...
> The re-translation of the Chinese version:
> 
> Any of the following sequences can be seen outside the CPU.
> 
...
> The re-translation of the Chinese version:
> 
> The second of which will almost certainly result in a malfunction, because
> the write comes after the read.
> 
> > 如果我们使用了编译器屏障 READ_ONCE 和 WRITE_ONCE，
> > 那就可以避免编译器对代码进行优化，此时生成的汇编指令跟代码是一样的，
> > CPU 应当对这些汇编指令提供一些最基本的保证：
> 
> The original English version:
> 
...
> The re-translation of the Chinese version:
> 
> If we employ compiler barriers such as READ_ONCE and WRITE_ONCE,
> we can protect the code from being optimized by the compiler, and the
> generated assembly instructions stay the same with the original code.
> CPU should provide some basic gurantees for these assembly instructions.
> 
> Thanks,
> Patrick
> 
...
> 
> 
> On Tue, Aug 15, 2023 at 12:02 PM Patrick Yingxi Pan
> <pyxchina92929@gmail.com> wrote:
> >
> > Hello Gang,
> >
> > Thanks for the hard work! It is a pretty huge patch. (The original one
> > as well ;-p )
> >
> > I have proposed some minor improvements on wording up until the text.
> > They may not be correct and are subject to discussion. I hope they
> > somewhat help.
> >
> > I will continue on to the text as soon as I get free time.
> >
> > Thanks,
> > Patrick
> >
...
> >
> >
> > On Mon, Aug 14, 2023 at 10:00 PM Gang Li <gang.li@linux.dev> wrote:
> > >

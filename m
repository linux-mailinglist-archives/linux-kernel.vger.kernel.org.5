Return-Path: <linux-kernel+bounces-49188-lists+linux-kernel=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-kernel@lfdr.de
Delivered-To: lists+linux-kernel@lfdr.de
Received: from ny.mirrors.kernel.org (ny.mirrors.kernel.org [IPv6:2604:1380:45d1:ec00::1])
	by mail.lfdr.de (Postfix) with ESMTPS id 0A85A8466F2
	for <lists+linux-kernel@lfdr.de>; Fri,  2 Feb 2024 05:21:39 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by ny.mirrors.kernel.org (Postfix) with ESMTPS id 1F9A51C248B9
	for <lists+linux-kernel@lfdr.de>; Fri,  2 Feb 2024 04:21:38 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 7F874F4EA;
	Fri,  2 Feb 2024 04:21:34 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=fail reason="signature verification failed" (2048-bit key) header.d=google.com header.i=@google.com header.b="Yr8S48qh"
Received: from mail-pl1-f182.google.com (mail-pl1-f182.google.com [209.85.214.182])
	(using TLSv1.2 with cipher ECDHE-RSA-AES128-GCM-SHA256 (128/128 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 539F6F9CE
	for <linux-kernel@vger.kernel.org>; Fri,  2 Feb 2024 04:21:27 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org; arc=none smtp.client-ip=209.85.214.182
ARC-Seal:i=1; a=rsa-sha256; d=subspace.kernel.org; s=arc-20240116;
	t=1706847690; cv=none; b=ZLDB4LJzzQZhd6CemTPMYE4ajb4f6CRJiqWo1dYWmdw9C+zmnuqy7mJ1y6e3JvqKw15XDJfmPX61b9L5IGANeLJgk/dBzg1kzy7UZaEFOoBO+uwEGp0pmjN4DuTxapda+msmSnDkLQbF91dq0V6ge/PC9WmR06JRf8u1cSCWbFw=
ARC-Message-Signature:i=1; a=rsa-sha256; d=subspace.kernel.org;
	s=arc-20240116; t=1706847690; c=relaxed/simple;
	bh=lpxYKCS7PTzPh/VeE0QaLdCj5Mz4LHs1B3W21ceeYjg=;
	h=MIME-Version:References:In-Reply-To:From:Date:Message-ID:Subject:
	 To:Cc:Content-Type; b=hhTG3VGsKxSQlV0cqWHPo9BEq6oFJ4IrnR6caccEXxjCT/zGSRklyijVBz0aaVbgJTU6xBRrjI4Z9cvxx39wrUBvY6kg54D+e9u+nuQQzB5gFXb8GZeZJrKUXTkvOtv3KCB3ufvOF6fopGkix5Lx5qcW4GVoeSwhAVFesW5ds3I=
ARC-Authentication-Results:i=1; smtp.subspace.kernel.org; dmarc=pass (p=reject dis=none) header.from=google.com; spf=pass smtp.mailfrom=google.com; dkim=pass (2048-bit key) header.d=google.com header.i=@google.com header.b=Yr8S48qh; arc=none smtp.client-ip=209.85.214.182
Authentication-Results: smtp.subspace.kernel.org; dmarc=pass (p=reject dis=none) header.from=google.com
Authentication-Results: smtp.subspace.kernel.org; spf=pass smtp.mailfrom=google.com
Received: by mail-pl1-f182.google.com with SMTP id d9443c01a7336-1d93b982761so117475ad.0
        for <linux-kernel@vger.kernel.org>; Thu, 01 Feb 2024 20:21:27 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=google.com; s=20230601; t=1706847686; x=1707452486; darn=vger.kernel.org;
        h=content-transfer-encoding:cc:to:subject:message-id:date:from
         :in-reply-to:references:mime-version:from:to:cc:subject:date
         :message-id:reply-to;
        bh=RA00L0Fb4tGLkygbbDhDkPEU9RP5hf0EQStXkxHh/Vs=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=1e100.net; s=20230601; t=1706847686; x=1707452486;
        h=content-transfer-encoding:cc:to:subject:message-id:date:from
         :in-reply-to:references:mime-version:x-gm-message-state:from:to:cc
         :subject:date:message-id:reply-to;
        bh=RA00L0Fb4tGLkygbbDhDkPEU9RP5hf0EQStXkxHh/Vs=;
        b=
X-Gm-Message-State: AOJu0YxXFntHXivsWx+QsiOxxbrwQeyQBEaRNNvwlF3mZrRvAFv0Hi5Q
	OQKm3hpkEL86MHg4wIujshmsdMamSW694xpyv4MqzSFhgRxUEFiBivY04NJF0CytFQjTZgBqG+b
	qJAd7XEekiIBh2e3/jQnRVmlH6UA33yzxQkE6
X-Google-Smtp-Source: AGHT+IG3gY0a5oeLqi5lANZ1WVcvICYEUhC+C4ZACjBvx0ugQsebJsoFoPMw2CUy0h5S65Ff5FkN0DfOGHTfAtGin5k=
X-Received: by 2002:a17:902:f544:b0:1d9:760d:31c1 with SMTP id
 h4-20020a170902f54400b001d9760d31c1mr6374plf.23.1706847685700; Thu, 01 Feb
 2024 20:21:25 -0800 (PST)
Precedence: bulk
...
Date: Thu, 1 Feb 2024 20:21:12 -0800
Message-ID: <CAP-5=fXygUWfqSb9x6gn6ER0MvBFOvuOPDNNo95THLK74dwXDg@mail.gmail.com>
...
On Thu, Feb 1, 2024 at 8:20=E2=80=AFPM Ian Rogers <irogers@google.com> wrot=
e:
>
...
> > Hi Ian,
> >
> > On Tue, Jan 2, 2024 at 9:07=E2=80=AFPM Ian Rogers <irogers@google.com> =
wrote:
> > >
...
> > > @@ -168,6 +168,7 @@ struct map *map__new(struct machine *machine, u64=
 start, u64 len,
> > >                 if (dso =3D=3D NULL)
> > >                         goto out_delete;
> > >
...
> > > -       for (map =3D maps__first(maps); map; map =3D map_rb_node__nex=
t(map))
> > > +static void check_invariants(const struct maps *maps __maybe_unused)
> > > +{
> > > +#ifndef NDEBUG
> > > +       assert(RC_CHK_ACCESS(maps)->nr_maps <=3D RC_CHK_ACCESS(maps)-=
>nr_maps_allocated);
> > > +       for (unsigned int i =3D 0; i < RC_CHK_ACCESS(maps)->nr_maps; =
i++) {
> > > +               struct map *map =3D RC_CHK_ACCESS(maps)->maps_by_addr=
ess[i];
> > > +
> > > +               /* Check map is well-formed. */
> > > +               assert(map__end(map) =3D=3D 0 || map__start(map) <=3D=
 map__end(map));
> > > +               /* Expect at least 1 reference count. */
> > > +               assert(refcount_read(map__refcnt(map)) > 0);
> > > +
> > > +               if (map__dso(map) && map__dso(map)->kernel)
> > > +                       assert(RC_CHK_EQUAL(map__kmap(map)->kmaps, ma=
ps));
> > > +
> > > +               if (i > 0) {
> > > +                       struct map *prev =3D RC_CHK_ACCESS(maps)->map=
s_by_address[i - 1];
> > > +
> > > +                       /* If addresses are sorted... */
> > > +                       if (RC_CHK_ACCESS(maps)->maps_by_address_sort=
ed) {
> > > +                               /* Maps should be in start address or=
der. */
> > > +                               assert(map__start(prev) <=3D map__sta=
rt(map));
> > > +                               /*
> > > +                                * If the ends of maps aren't broken =
(during
> > > +                                * construction) then they should be =
ordered
> > > +                                * too.
> > > +                                */
> > > +                               if (!RC_CHK_ACCESS(maps)->ends_broken=
) {
> > > +                                       assert(map__end(prev) <=3D ma=
p__end(map));
> > > +                                       assert(map__end(prev) <=3D ma=
p__start(map) ||
> > > +                                              map__start(prev) =3D=
...
> > > +               for (unsigned int i =3D 0; i < RC_CHK_ACCESS(maps)->n=
r_maps; i++) {
> > > +                       struct map *map =3D RC_CHK_ACCESS(maps)->maps=
_by_name[i];
> > >
> > > -#define maps__for_each_entry_safe(maps, map, next) \
> > > -       for (map =3D maps__first(maps), next =3D map_rb_node__next(ma=
p); map; \
> > > -            map =3D next, next =3D map_rb_node__next(map))
> > > +                       /*
> > > +                        * Maps by name maps should be in maps_by_add=
ress, so
...
> > > +static void maps__set_maps_by_name(struct maps *maps, struct map **n=
ew)
> > >  {
...
> > >  }
> > >
> > > -static struct map_rb_node *map_rb_node__next(struct map_rb_node *nod=
e)
...
> > > -       if (!next)
> > > -               return NULL;
> > > +static void maps__set_maps_by_address_sorted(struct maps *maps, bool=
 value)
> > > +{
...
> > >  }
> > >
> > > -static struct map_rb_node *maps__find_node(struct maps *maps, struct=
 map *map)
> > > +static void maps__set_maps_by_name_sorted(struct maps *maps, bool va=
lue)
> > >  {
...
> > > -               if (rb_node->RC_CHK_ACCESS(map) =3D=3D RC_CHK_ACCESS(=
map))
> > > -                       return rb_node;
> > > -       }
> > > -       return NULL;
...
> > > +{
> > > +       /*
> > > +        * When the lock is acquired or released the maps invariants =
should
> > > +        * hold.
> > > +        */
...
> > >  {
> > > -       /*
> > > -        * Free everything to try to do it from the rbtree in the nex=
t search
> > > -        */
...
> > > -                       kmap->kmaps =3D maps;
> > > -               else
> > > -                       pr_err("Internal error: kernel dso with non k=
ernel map\n");
> > > -       }
> > > +       return result;
> > > +}
...
> > > +{
> > >         /*
> > > -        * If we already performed some search by name, then we need =
to add the just
...
> > > -               if (maps__nr_maps(maps) > RC_CHK_ACCESS(maps)->nr_map=
s_allocated) {
> > > -                       int nr_allocate =3D maps__nr_maps(maps) * 2;
> > > -                       struct map **maps_by_name =3D realloc(maps__m=
aps_by_name(maps),
> > > -                                                           nr_alloca=
te * sizeof(map));
> > > +       for (unsigned int i =3D 0; i < maps__nr_maps(maps); i++)
...
> > > +}
> > >
> > > -                       RC_CHK_ACCESS(maps)->maps_by_name =3D maps_by=
_name;
> > > -                       RC_CHK_ACCESS(maps)->nr_maps_allocated =3D nr=
_allocate;
...
> > > +                       /* Ensure maps with the same addresses have a=
 fixed order. */
> > > +                       if (RC_CHK_ACCESS(map_a) =3D=3D RC_CHK_ACCESS=
(map_b))
> > > +                               return 0;
> > > +                       return (intptr_t)RC_CHK_ACCESS(map_a) > (intp=
tr_t)RC_CHK_ACCESS(map_b)
> > > +                               ? 1 : -1;
> > >                 }
> > > -               maps__maps_by_name(maps)[maps__nr_maps(maps) - 1] =3D=
 map__get(map);
...
> > >  }
> > >
> > > -static void __maps__remove(struct maps *maps, struct map_rb_node *rb=
_node)
...
> > > -               map__put(pos->map);
> > > -               free(pos);
> > > +       if (ret =3D=3D 0 && RC_CHK_ACCESS(map_a) !=3D RC_CHK_ACCESS(m=
ap_b)) {
...
> > > +
> > > +               if (!maps_by_name) {
> > > +                       maps_by_name =3D malloc(RC_CHK_ACCESS(maps)->=
nr_maps_allocated *
> > > +                                       sizeof(*maps_by_name));
> > > +                       if (!maps_by_name)
> > > +                               err =3D -ENOMEM;
...
> > > +
> > > +                               maps__set_maps_by_name(maps, maps_by_=
name);
> > > +                               for (unsigned int i =3D 0; i < n; i++=
)
> > > +                                       maps_by_name[i] =3D map__get(=
maps_by_address[i]);
> > > +                       }
> > > +               }
> > > +               if (!err) {
...
> > > +       if (maps__maps_by_address_sorted(maps)) {
> > > +               struct map **mapp =3D
> > > +                       bsearch(&map, maps__maps_by_address(maps), ma=
ps__nr_maps(maps),
...
> > > +               for (unsigned int i =3D 0; i < maps__nr_maps(maps); i=
++) {
> > > +                       if (RC_CHK_ACCESS(maps_by_address[i]) =3D=3D =
RC_CHK_ACCESS(map))
> > > +                               return i;
> > > +               }
> > > +       }
...
> > >
> > > -struct maps *maps__new(struct machine *machine)
> > > +static unsigned int maps__by_name_index(const struct maps *maps, con=
st struct map *map)
> > >  {
> > > -       struct maps *result;
> > > -       RC_STRUCT(maps) *maps =3D zalloc(sizeof(*maps));
...
> > > +       if (maps__maps_by_name_sorted(maps)) {
> > > +               struct map **mapp =3D
> > > +                       bsearch(&map, maps_by_name, maps__nr_maps(map=
s),
...
> > > +               for (unsigned int i =3D 0; i < maps__nr_maps(maps); i=
++) {
> > > +                       if (RC_CHK_ACCESS(maps_by_name[i]) =3D=3D RC_=
CHK_ACCESS(map))
> > > +                               return i;
> > > +               }
> > > +       }
...
> > > +       unsigned int nr_allocate =3D RC_CHK_ACCESS(maps)->nr_maps_all=
ocated;
> > > +
> > > +       if (nr_maps + 1 > nr_allocate) {
> > > +               nr_allocate =3D !nr_allocate ? 32 : nr_allocate * 2;
> > > +
> > > +               maps_by_address =3D realloc(maps_by_address, nr_alloc=
ate * sizeof(new));
> > > +               if (!maps_by_address)
> > > +                       return -ENOMEM;
> > > +
...
> > > +               if (maps_by_name) {
> > > +                       maps_by_name =3D realloc(maps_by_name, nr_all=
ocate * sizeof(new));
> > > +                       if (!maps_by_name) {
> > > +                               /*
> > > +                                * If by name fails, just disable by =
name and it will
...
> > > +               }
> > > +               RC_CHK_ACCESS(maps)->nr_maps_allocated =3D nr_allocat=
e;
> > > +       }
...
> > > +
> > > +       /*
> > > +        * Recompute if things are sorted. If things are inserted in =
a sorted
...
> > > +               maps__set_maps_by_address_sorted(maps, true);
> > > +               maps__set_maps_by_name_sorted(maps, maps_by_name !=3D=
 NULL);
> > > +       } else {
> > > +               /* Sorted if maps were already sorted and this map st=
arts after the last one. */
> > > +               maps__set_maps_by_address_sorted(maps,
> > > +                       maps__maps_by_address_sorted(maps) &&
> > > +                       map__end(maps_by_address[nr_maps - 2]) <=3D m=
ap__start(new));
...
> > > +
> > > +       if (maps_by_name) {
> > > +               unsigned int name_idx =3D maps__by_name_index(maps, m=
ap);
> > > +
> > > +               map__put(maps_by_name[name_idx]);
> > > +               memmove(&maps_by_name[name_idx],
> > > +                       &maps_by_name[name_idx + 1],
> > > +                       (nr_maps - name_idx - 1) *  sizeof(*maps_by_n=
ame));
> > > +       }
> > >
> > > -       if (RC_CHK_GET(result, maps))
...
> > > +               down_read(maps__lock(maps));
> > > +               if (maps__maps_by_address_sorted(maps)) {
> > > +                       struct map **maps_by_address =3D maps__maps_b=
y_address(maps);
> > > +                       unsigned int n =3D maps__nr_maps(maps);
> > > +
> > > +                       for (unsigned int i =3D 0; i < n; i++) {
...
> > > +                       i++;
> > >         }
> > > -       if (maps__maps_by_name(maps) && start_nr_maps !=3D maps__nr_m=
aps(maps))
> > > -               __maps__free_maps_by_name(maps);
> > > -
> > >         up_write(maps__lock(maps));
> > >  }
> > >
> > > @@ -300,7 +491,7 @@ struct symbol *maps__find_symbol(struct maps *map=
s, u64 addr, struct map **mapp)
> > >         /* Ensure map is loaded before using map->map_ip */
> > >         if (map !=3D NULL && map__load(map) >=3D 0) {
> > >                 if (mapp !=3D NULL)
> > > -                       *mapp =3D map;
> > > +                       *mapp =3D map; // TODO: map_put on else path =
when find returns a get.
> > >                 return map__find_symbol(map, map__map_ip(map, addr));
> > >         }
> > >
> > > @@ -348,7 +539,7 @@ int maps__find_ams(struct maps *maps, struct addr=
_map_symbol *ams)
> > >         if (ams->addr < map__start(ams->ms.map) || ams->addr >=3D map=
__end(ams->ms.map)) {
> > >                 if (maps =3D=3D NULL)
> > >                         return -1;
...
> > >                         return -1;
> > >         }
> > > @@ -393,24 +584,28 @@ size_t maps__fprintf(struct maps *maps, FILE *f=
p)
> > >   * Find first map where end > map->start.
> > >   * Same as find_vma() in kernel.
> > >   */
> > > -static struct rb_node *first_ending_after(struct maps *maps, const s=
truct map *map)
> > > +static unsigned int first_ending_after(struct maps *maps, const stru=
ct map *map)
> > >  {
> > > -       struct rb_root *root;
> > > -       struct rb_node *next, *first;
> > > +       struct map **maps_by_address =3D maps__maps_by_address(maps);
> > > +       int low =3D 0, high =3D (int)maps__nr_maps(maps) - 1, first =
=3D high + 1;
> > > +
> > > +       assert(maps__maps_by_address_sorted(maps));
> > > +       if (low <=3D high && map__end(maps_by_address[0]) > map__star=
t(map))
> > > +               return 0;
> > >
...
> > > -       first =3D NULL;
> > > -       while (next) {
> > > -               struct map_rb_node *pos =3D rb_entry(next, struct map=
_rb_node, rb_node);
> > > +       while (low <=3D high) {
...
> > >         return first;
> > >  }
> > > @@ -419,171 +614,249 @@ static struct rb_node *first_ending_after(str=
uct maps *maps, const struct map *m
> > >   * Adds new to maps, if new overlaps existing entries then the exist=
ing maps are
> > >   * adjusted or removed so that new fits without overlapping any entr=
ies.
> > >   */
> > > -int maps__fixup_overlap_and_insert(struct maps *maps, struct map *ne=
w)
> > > +static int __maps__fixup_overlap_and_insert(struct maps *maps, struc=
t map *new)
> > >  {
> > > -
> > > -       struct rb_node *next;
...
> > > +       maps_by_address =3D maps__maps_by_address(maps);
> > > +       /*
> > > +        * Iterate through entries where the end of the existing entr=
y is
> > > +        * greater-than the new map's start.
> > > +        */
> > > +       for (unsigned int i =3D first_ending_after(maps, new); i < ma=
ps__nr_maps(maps); ) {
...
> > >                  * Maps are ordered by start: next will not overlap f=
or sure.
> > >                  */
...
> > > -
> > > -                       if (use_browser) {
> > > -                               pr_debug("overlapping maps in %s (dis=
able tui for more info)\n",
> > > -                                        map__dso(new)->name);
> > > -                       } else {
...
> > > -                       }
> > > +               if (use_browser) {
> > > +                       pr_debug("overlapping maps in %s (disable tui=
 for more info)\n",
> > > +                               map__dso(new)->name);
...
> > > -               rb_erase_init(&pos->rb_node, maps__entries(maps));
> > >                 /*
> > >                  * Now check if we need to create new maps for areas =
not
...
> > > +                       /* Map starts within existing map. Need to sh=
orten the existing map. */
...
> > > +                       /* The new map isn't as long as the existing =
map. */
...
> > > -                       map__add_pgoff(after, map__end(new) - map__st=
art(pos->map));
...
> > > -               free(pos);
> > > +               /*
> > > +                * If adding one entry, for `before` or `after`, we c=
an replace
> > > +                * the existing entry. If both `before` and `after` a=
re
> > > +                * necessary than an insert is needed. If the existin=
g entry
> > > +                * entirely overlaps the existing entry it can just b=
e removed.
> > > +                */
> > > +               if (before) {
...
> > > +                               map__put(after);
> > > +                               if (!maps__maps_by_address_sorted(map=
s)) {
> > > +                                       /*
> > > +                                        * Sorting broken so invarian=
ts don't
...
> > > +                               }
> > > +                               /*
> > > +                                * Maps are still ordered, skip after=
 and go to
...
> > > +                       __maps__remove(maps, pos);
> > > +                       /*
> > > +                        * Maps are ordered but no need to increase `=
i` as the
...
> > > -                */
> > > -               ret =3D (int)map__start(map_a) - (int)map__start(map_=
b);
> > > -               if (ret =3D=3D 0)
> > > -                       ret =3D (int)((intptr_t)map_a - (intptr_t)map=
_b);
...
> > > +               if (maps__maps_by_address_sorted(maps)) {
> > > +                       struct map **mapp =3D
> > > +                               bsearch(&ip, maps__maps_by_address(ma=
ps), maps__nr_maps(maps),
...
> > > @@ -593,126 +866,113 @@ static int map__strcmp_name(const void *name,=
 const void *b)
...
> > > -{
> > > -       qsort(maps__maps_by_name(maps), maps__nr_maps(maps), sizeof(s=
truct map *), map__strcmp);
> > > -}
> > > -
...
> > > -}
> > > -
> > > -static struct map *__maps__find_by_name(struct maps *maps, const cha=
r *name)
...
> > > +               unsigned int i;
> > >
> > > -       mapp =3D bsearch(name, maps__maps_by_name(maps), maps__nr_map=
s(maps),
...
> > > +               if (i < maps__nr_maps(maps) && maps__maps_by_name(map=
s)) {
> > > +                       struct dso *dso =3D map__dso(maps__maps_by_na=
me(maps)[i]);
> > >
> > > +                       if (dso && strcmp(dso->short_name, name) =3D=
=3D 0) {
...
> > > +                                       sizeof(*mapp), map__strcmp_na=
me);
> > >
> > > -               if (strcmp(dso->short_name, name) =3D=3D 0) {
> > > -                       map =3D RC_CHK_ACCESS(maps)->last_search_by_n=
ame;
> > > -                       goto out_unlock;
> > > +                       if (mapp) {
...
;
> > > +                               RC_CHK_ACCESS(maps)->last_search_by_n=
ame_idx =3D i;
> > > +                       }
> > > +                       done =3D true;
> > >                 }
> > > -       }
> > > -       /*
> > > -        * If we have maps->maps_by_name, then the name isn't in the =
rbtree,
> > > -        * as maps->maps_by_name mirrors the rbtree when lookups by n=
ame are
> > > -        * made.
> > > -        */
...
> > > -               dso =3D map__dso(map);
> > > -               if (strcmp(dso->short_name, name) =3D=3D 0) {
> > > -                       RC_CHK_ACCESS(maps)->last_search_by_name =3D =
map;
> > > -                       goto out_unlock;
> > > +               up_read(maps__lock(maps));
> > > +               if (!done) {
...
> > > +                               /*
> > > +                                * Memory allocation failed do linear=
 search
...
> > > +
> > > +                               down_read(maps__lock(maps));
> > > +                               maps_by_address =3D  maps__maps_by_ad=
dress(maps);
> > > +                               n =3D maps__nr_maps(maps);
> > > +                               for (i =3D 0; i < n; i++) {
> > > +                                       struct map *pos =3D maps_by_a=
ddress[i];
> > > +                                       struct dso *dso =3D map__dso(=
pos);
> > > +
> > > +                                       if (dso && strcmp(dso->short_=
name, name) =3D=3D 0) {
> > > +                                               result =3D pos; // TO=
DO: map__get
> > > +                                               break;
...
> > > +               result =3D maps__maps_by_address(maps)[i]; // TODO: m=
ap__get
> > >
> > > -       return NULL;
> > > +       up_read(maps__lock(maps));
...
> > > -               if (prev && (!map__end(prev->map) || map__end(prev->m=
ap) > map__start(curr->map)))
...
> > >
> > > -               prev =3D curr;
> > > +               if (!map__end(prev) || map__end(prev) > map__start(cu=
rr))
...
> > > -       rb_node =3D first ? rb_entry(first, struct map_rb_node, rb_no=
de) : NULL;
> > > -       overlaps =3D rb_node && map__start(rb_node->map) < map__end(n=
ew_map);
> > > -       up_read(maps__lock(kmaps));
> > > +               up_read(maps__lock(kmaps));
> > > +
> > > +               /* First after binary search requires sorted maps. So=
rt and try again. */
...
> > > +           map__start(kmaps_maps_by_address[first_after_]) >=3D map_=
_end(new_map)) {
...
> > > +       /* Plain insert with a read-lock failed, try again now with t=
he write lock. */
...
> > > -                                *       |old....| ->       |old....|
> > > -                                */
> > > -                               map__set_end(new_map, map__start(old_=
map));
> > > -                       } else {
> > > -                               /*
> > > -                                * |new.............| -> |new..|     =
...
.|
> > > -                                */
> > > -                               struct map_list_node *m =3D map_list_=
node__new();
...
>
> > > +       }
> > > +       /* Array to merge into, possibly 1 more for the sake of new_m=
ap. */
> > > +       merged_nr_maps_allocated =3D RC_CHK_ACCESS(kmaps)->nr_maps_al=
located;
> > > +       if (kmaps__nr_maps + 1 =3D=3D merged_nr_maps_allocated)
> > > +               merged_nr_maps_allocated++;
> > > +
> > > +       merged_maps_by_address =3D malloc(merged_nr_maps_allocated * =
sizeof(*merged_maps_by_address));
> > > +       if (!merged_maps_by_address) {
> > > +               up_write(maps__lock(kmaps));
> > > +               return -ENOMEM;
> > > +       }
> > > +       RC_CHK_ACCESS(kmaps)->maps_by_address =3D merged_maps_by_addr=
ess;
...
> > > +       RC_CHK_ACCESS(kmaps)->nr_maps_allocated =3D merged_nr_maps_al=
located;
> >
...
> > > +               merged_maps_by_address[i] =3D map__get(kmaps_maps_by_=
address[i]);
> > >
...
> > >
> > > -                               map__set_end(m->map, map__start(old_m=
ap));
> > > -                               list_add_tail(&m->node, &merged);
> > > -                               map__add_pgoff(new_map, map__end(old_=
map) - map__start(new_map));
> > > -                               map__set_start(new_map, map__end(old_=
map));
> > > -                       }
> > > -               } else {
> > > -                       /*
...
> > > -               }
> > > -       }
> > > +       /* Add the new map, it will be split when the later overlappi=
ng mappings are added. */
> > > +       __maps__insert(kmaps, new_map);
> > >
> > > -out:
> > > -       while (!list_empty(&merged)) {
> > > -               struct map_list_node *old_node;
> > > +       /* Insert mappings after new_map, splitting new_map in the pr=
ocess. */
> > > +       for (unsigned int i =3D first_after_; i < kmaps__nr_maps; i++=
)
> > > +               __maps__fixup_overlap_and_insert(kmaps, kmaps_maps_by=
_address[i]);
> > >
> > > -               old_node =3D list_entry(merged.next, struct map_list_=
node, node);
...
> > > @@ -25,21 +25,56 @@ static inline struct map_list_node *map_list_node=
__new(void)
...
> > > + *
> > > + * Sorting is done with the write lock, iteration and binary searchi=
ng happens
> > > + * under the read lock requiring being sorted. There is a race betwe=
en sorting
> > > + * releasing the write lock and acquiring the read lock for iteratio=
n/searching
> > > + * where another thread could insert and break the sorting of the ma=
ps. In
> > > + * practice inserting maps should be rare meaning that the race shou=
ldn't lead
...
> > > +       /**
> > > +        * @maps_by_address: array of maps sorted by their starting a=
ddress if
> > > +        * maps_by_address_sorted is true.
> > > +        */
> > > +       struct map       **maps_by_address;
> > > +       /**
> > > +        * @maps_by_name: optional array of maps sorted by their dso =
name if
...
> > > +       refcount_t       refcnt;
> > > +       /**
> > > +        * @nr_maps: number of maps_by_address, and possibly maps_by_=
name,
> > > +        * entries that contain maps.
> > > +        */
> > > +       unsigned int     nr_maps;
> > > +       /**
> > > +        * @nr_maps_allocated: number of entries in maps_by_address a=
nd possibly
> > > +        * maps_by_name.
> > > +        */
> > > +       unsigned int     nr_maps_allocated;
> > > +       /**
> > > +        * @last_search_by_name_idx: cache of last found by name entr=
y's index
...
> > > +       /** @ends_broken: does the map contain a map where end values=
 are unset/unsorted? */
> > > +       bool             ends_broken;
> > >  };
> > >
...
> > >
> > > +struct map *maps__find(struct maps *maps, u64 addr);
> > >  struct symbol *maps__find_symbol(struct maps *maps, u64 addr, struct=
 map **mapp);
> > >  struct symbol *maps__find_symbol_by_name(struct maps *maps, const ch=
ar *name, struct map **mapp);
> > >
> > > @@ -117,8 +153,6 @@ struct map *maps__find_next_entry(struct maps *ma=
ps, struct map *map);
> > >
> > >  int maps__merge_in(struct maps *kmaps, struct map *new_map);
> > >

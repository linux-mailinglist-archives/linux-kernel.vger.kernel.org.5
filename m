Return-Path: <linux-kernel+bounces-1481-lists+linux-kernel=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-kernel@lfdr.de
Delivered-To: lists+linux-kernel@lfdr.de
Received: from ny.mirrors.kernel.org (ny.mirrors.kernel.org [147.75.199.223])
	by mail.lfdr.de (Postfix) with ESMTPS id BDE40814F4D
	for <lists+linux-kernel@lfdr.de>; Fri, 15 Dec 2023 18:56:10 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by ny.mirrors.kernel.org (Postfix) with ESMTPS id E24E71C24012
	for <lists+linux-kernel@lfdr.de>; Fri, 15 Dec 2023 17:56:09 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id CD9564597D;
	Fri, 15 Dec 2023 17:54:55 +0000 (UTC)
X-Original-To: linux-kernel@vger.kernel.org
Received: from smtp.kernel.org (aws-us-west-2-korg-mail-1.web.codeaurora.org [10.30.226.201])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 6111F41857;
	Fri, 15 Dec 2023 17:54:55 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id D3EF7C433BA;
	Fri, 15 Dec 2023 17:54:54 +0000 (UTC)
Received: from rostedt by gandalf with local (Exim 4.97)
	(envelope-from <rostedt@goodmis.org>)
	id 1rECPY-00000002wPn-3i4J;
	Fri, 15 Dec 2023 12:55:44 -0500
Message-ID: <20231215175544.666293237@goodmis.org>
User-Agent: quilt/0.67
Date: Fri, 15 Dec 2023 12:55:07 -0500
...
Subject: [PATCH v4 05/15] ring-buffer: Read and write to ring buffers with custom sub buffer
 size
...
@@ -5475,8 +5489,12 @@ void ring_buffer_free_read_page(struct trace_buffer *buffer, int cpu, void *data
...
+	if (page_ref_count(page) > 1 || data_page->order != buffer->subbuf_order)
 	...
 	local_irq_save(flags);
@@ -5491,7 +5509,8 @@ void ring_buffer_free_read_page(struct trace_buffer *buffer, int cpu, void *data
 	local_irq_restore(flags);
...
@@ -5512,9 +5531,10 @@ EXPORT_SYMBOL_GPL(ring_buffer_free_read_page);
...
@@ -5529,7 +5549,8 @@ EXPORT_SYMBOL_GPL(ring_buffer_free_read_page);
...
@@ -5554,10 +5575,12 @@ int ring_buffer_read_page(struct trace_buffer *buffer,
...
@@ -5651,11 +5674,11 @@ int ring_buffer_read_page(struct trace_buffer *buffer,
 		/* swap the pages */
 		rb_init_page(bpage);
 		bpage = reader->page;
...
@@ -5700,6 +5723,18 @@ int ring_buffer_read_page(struct trace_buffer *buffer,
 }
...
index 88d7feca0a25..d4d4e7c7357f 100644
--- a/kernel/trace/trace.c
+++ b/kernel/trace/trace.c
@@ -8293,6 +8293,8 @@ tracing_buffers_read(struct file *filp, char __user *ubuf,
 {
...
@@ -8304,6 +8306,8 @@ tracing_buffers_read(struct file *filp, char __user *ubuf,
 		return -EBUSY;
 #endif
...
+
 	if (!info->spare) {
 		info->spare = ring_buffer_alloc_read_page(iter->array_buffer->buffer,
 							  iter->cpu_file);
@@ -8318,13 +8322,13 @@ tracing_buffers_read(struct file *filp, char __user *ubuf,
 		return ret;
...
@@ -8345,11 +8349,11 @@ tracing_buffers_read(struct file *filp, char __user *ubuf,
...
@@ -8460,6 +8464,7 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
...
@@ -8468,13 +8473,14 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
 		return -EBUSY;
 #endif
...
@@ -8484,7 +8490,7 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
 	trace_access_lock(iter->cpu_file);
...
@@ -8505,7 +8511,7 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
 		}
...
@@ -8514,14 +8520,14 @@ tracing_buffers_splice_read(struct file *file, loff_t *ppos,
 			break;
 		}

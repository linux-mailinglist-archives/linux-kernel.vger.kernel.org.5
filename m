...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 3037479DE6A
	for <lists+linux-kernel@lfdr.de>; Wed, 13 Sep 2023 05:00:14 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S238211AbjIMDAP (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 12 Sep 2023 23:00:15 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:48780 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S232476AbjIMDAH (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 12 Sep 2023 23:00:07 -0400
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 6C5FFCEB;
        Tue, 12 Sep 2023 20:00:03 -0700 (PDT)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id BE215C433CD;
        Wed, 13 Sep 2023 03:00:02 +0000 (UTC)
Received: from rostedt by gandalf with local (Exim 4.96)
        (envelope-from <rostedt@goodmis.org>)
        id 1qgG73-000QQ4-33;
        Tue, 12 Sep 2023 23:00:21 -0400
Message-ID: <20230913030021.762361768@goodmis.org>
User-Agent: quilt/0.66
Date:   Tue, 12 Sep 2023 22:58:57 -0400
...
Subject: [PATCH 2/3] eventfs: Remove eventfs_file and just use eventfs_inode
...
---
 ...
 9 files changed, 688 insertions(+), 585 deletions(-)

diff --git a/fs/tracefs/event_inode.c b/fs/tracefs/event_inode.c
index 9f64e7332796..7e6e77a73900 100644
...
@@ -92,16 +53,9 @@ static const struct file_operations eventfs_file_operations = {
  * @data: something that the caller will want to get to later on.
...
@@ -117,6 +71,7 @@ static struct dentry *create_file(const char *name, umode_t mode,
...
+	WARN_ON_ONCE(!parent);
 	...
 	if (IS_ERR(dentry))
@@ -141,20 +96,11 @@ static struct dentry *create_file(const char *name, umode_t mode,
...
+static struct dentry *create_dir(const char *name, struct dentry *parent)
 {
 	...
 	struct dentry *dentry;
@@ -171,7 +117,6 @@ static struct dentry *create_dir(const char *name, struct dentry *parent, void *
...
@@ -184,18 +129,18 @@ static struct dentry *create_dir(const char *name, struct dentry *parent, void *
 }
...
@@ -206,9 +151,9 @@ void eventfs_set_ef_status_free(struct tracefs_inode *ti, struct dentry *dentry)
 		ei = ti->private;
...
@@ -217,11 +162,13 @@ void eventfs_set_ef_status_free(struct tracefs_inode *ti, struct dentry *dentry)
...
@@ -232,68 +179,162 @@ void eventfs_set_ef_status_free(struct tracefs_inode *ti, struct dentry *dentry)
...
+		/* lookup does not need to up the ref count */
 		if (!lookup)
 			dget(dentry);
 		mutex_unlock(&eventfs_mutex);
@@ -301,42 +342,50 @@ create_dentry(struct eventfs_file *ef, struct dentry *parent, bool lookup)
 	}
 	mutex_unlock(&eventfs_mutex);
...
+		WARN_ON_ONCE(!ei->is_freed);
 	...
 	mutex_unlock(&eventfs_mutex);
 	if (invalidate)
@@ -348,50 +397,70 @@ create_dentry(struct eventfs_file *ef, struct dentry *parent, bool lookup)
 	return invalidate ? NULL : dentry;
 }
...
@@ -403,11 +472,12 @@ static struct dentry *eventfs_root_lookup(struct inode *dir,
  */
...
+	int i;
 	...
 	ti = get_tracefs(inode);
 	if (!(ti->flags & TRACEFS_EVENT_INODE))
@@ -415,10 +485,18 @@ static int eventfs_release(struct inode *inode, struct file *file)
...
+		dentry = ei->d_children[i];
 		...
 		if (dentry)
 			dput(dentry);
@@ -430,94 +508,140 @@ static int eventfs_release(struct inode *inode, struct file *file)
 /**
...
+						int size, void *data)
 {
 	...
 	struct eventfs_inode *ei;
@@ -528,19 +652,32 @@ struct dentry *eventfs_create_events_dir(const char *name,
 		return NULL;
...
@@ -555,193 +692,41 @@ struct dentry *eventfs_create_events_dir(const char *name,
...
@@ -753,62 +738,68 @@ static void eventfs_remove_rec(struct eventfs_file *ef, struct list_head *head,
...
@@ -823,8 +814,8 @@ void eventfs_remove(struct eventfs_file *ef)
 		mutex_lock(&eventfs_mutex);

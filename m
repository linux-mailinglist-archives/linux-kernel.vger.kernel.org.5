Return-Path: <linux-kernel+bounces-1478-lists+linux-kernel=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-kernel@lfdr.de
Delivered-To: lists+linux-kernel@lfdr.de
Received: from sy.mirrors.kernel.org (sy.mirrors.kernel.org [147.75.48.161])
	by mail.lfdr.de (Postfix) with ESMTPS id 7C8F2814F4A
	for <lists+linux-kernel@lfdr.de>; Fri, 15 Dec 2023 18:55:38 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by sy.mirrors.kernel.org (Postfix) with ESMTPS id 00C89B22FE3
	for <lists+linux-kernel@lfdr.de>; Fri, 15 Dec 2023 17:55:36 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 0E8893FE2C;
	Fri, 15 Dec 2023 17:54:55 +0000 (UTC)
X-Original-To: linux-kernel@vger.kernel.org
Received: from smtp.kernel.org (aws-us-west-2-korg-mail-1.web.codeaurora.org [10.30.226.201])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 9DEDE3DB86;
	Fri, 15 Dec 2023 17:54:54 +0000 (UTC)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 0C83EC433CC;
	Fri, 15 Dec 2023 17:54:54 +0000 (UTC)
Received: from rostedt by gandalf with local (Exim 4.97)
	(envelope-from <rostedt@goodmis.org>)
	id 1rECPY-00000002wOJ-01N3;
	Fri, 15 Dec 2023 12:55:44 -0500
Message-ID: <20231215175543.785678505@goodmis.org>
...
Subject: [PATCH v4 02/15] ring-buffer: Page size per ring buffer
...
@@ -1646,7 +1645,13 @@ struct trace_buffer *__ring_buffer_alloc(unsigned long size, unsigned flags,
...
+ * Minimum size is 2 * buffer->subbuf_size.
  ...
  */
@@ -1987,7 +1992,7 @@ int ring_buffer_resize(struct trace_buffer *buffer, unsigned long size,
...
@@ -2475,13 +2481,13 @@ rb_reset_tail(struct ring_buffer_per_cpu *cpu_buffer,
...
@@ -2509,7 +2515,7 @@ rb_reset_tail(struct ring_buffer_per_cpu *cpu_buffer,
...
@@ -2524,19 +2530,19 @@ rb_reset_tail(struct ring_buffer_per_cpu *cpu_buffer,
 	}
...
@@ -3409,7 +3415,7 @@ __rb_reserve_next(struct ring_buffer_per_cpu *cpu_buffer,
...
@@ -3533,7 +3539,7 @@ rb_reserve_next_event(struct trace_buffer *buffer,
...
@@ -3608,7 +3614,7 @@ ring_buffer_lock_reserve(struct trace_buffer *buffer, unsigned long length)
...
@@ -3758,7 +3764,7 @@ int ring_buffer_write(struct trace_buffer *buffer,
...
+	if (length > buffer->max_data_size)
 	...
 	if (unlikely(trace_recursive_lock(cpu_buffer)))
@@ -4338,6 +4344,7 @@ static struct buffer_page *
...
+	unsigned long bsize = READ_ONCE(cpu_buffer->buffer->subbuf_size);
 	...
 	unsigned long flags;
 	int nr_loops = 0;
@@ -4473,7 +4480,7 @@ rb_get_reader_page(struct ring_buffer_per_cpu *cpu_buffer)
...
+		if (likely(!reader || rb_page_write(reader) <= bsize))
 		...
 		udelay(1);
@@ -4917,7 +4924,8 @@ ring_buffer_read_prepare(struct trace_buffer *buffer, int cpu, gfp_t flags)
 		return NULL;
...
+	iter->event = kmalloc(iter->event_size, flags);
 	if (!iter->event) {
 		kfree(iter);
 		return NULL;
@@ -5035,14 +5043,14 @@ unsigned long ring_buffer_size(struct trace_buffer *buffer, int cpu)
 {
 	/*
...
@@ -5056,8 +5064,8 @@ unsigned long ring_buffer_max_event_size(struct trace_buffer *buffer)
 {
...
@@ -5663,7 +5671,7 @@ int ring_buffer_read_page(struct trace_buffer *buffer,
...
@@ -5675,8 +5683,8 @@ int ring_buffer_read_page(struct trace_buffer *buffer,
 	/*
...
index b157f0db397c..45ed9cb5399b 100644
...
index 562b32696d28..614dfb1929e6 100644

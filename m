Return-Path: <linux-kernel+bounces-138080-lists+linux-kernel=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-kernel@lfdr.de
Delivered-To: lists+linux-kernel@lfdr.de
Received: from ny.mirrors.kernel.org (ny.mirrors.kernel.org [147.75.199.223])
	by mail.lfdr.de (Postfix) with ESMTPS id 7C90989EC41
	for <lists+linux-kernel@lfdr.de>; Wed, 10 Apr 2024 09:37:52 +0200 (CEST)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by ny.mirrors.kernel.org (Postfix) with ESMTPS id A9A6E1C2115F
	for <lists+linux-kernel@lfdr.de>; Wed, 10 Apr 2024 07:37:51 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id D8F9513D270;
	Wed, 10 Apr 2024 07:37:46 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=pass (1024-bit key) header.d=linux.dev header.i=@linux.dev header.b="Id6sA/GM"
Received: from out-174.mta0.migadu.com (out-174.mta0.migadu.com [91.218.175.174])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id A69E8C2D6
	for <linux-kernel@vger.kernel.org>; Wed, 10 Apr 2024 07:37:42 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org; arc=none smtp.client-ip=91.218.175.174
ARC-Seal:i=1; a=rsa-sha256; d=subspace.kernel.org; s=arc-20240116;
	t=1712734665; cv=none; b=oaCgJWtzFU96dcC2S2HPHdj+mJJ/MnMbFPay9igcMNRgTy0SpdQNwUva4xAQtxZhwIRaDntk8HuyN47TjZhUvoPrEnEYNSRZqXwVkFKAzm3Nk/PpFCPHdqznMFgFmpIMn5BBoLO6OumePQwY0pE7LkJrz06ZYT3ANx0QL8W6hQc=
ARC-Message-Signature:i=1; a=rsa-sha256; d=subspace.kernel.org;
	s=arc-20240116; t=1712734665; c=relaxed/simple;
	bh=7o6bo6Z32jGn3Z3o2zDSxj5FFDf6qH9cw49nVjx36KA=;
	h=Content-Type:Mime-Version:Subject:From:In-Reply-To:Date:Cc:
	 Message-Id:References:To; b=I9rVe9wpu8OEgk9qkSjg57lFRKKQZNb6SGj+aZtE7T8RdpE84vPrMEG0L6bjxSQizvdQQSeAgW7K3jCNELZ06NG+8Yae2rUPcOjLNUb1iqc1Cgg3cQRzZyrrMuDyllxpqEsv8mcQ2yGmPktcMlzrHOvDiUsI03+z5/F5ZiLDMPA=
ARC-Authentication-Results:i=1; smtp.subspace.kernel.org; dmarc=pass (p=none dis=none) header.from=linux.dev; spf=pass smtp.mailfrom=linux.dev; dkim=pass (1024-bit key) header.d=linux.dev header.i=@linux.dev header.b=Id6sA/GM; arc=none smtp.client-ip=91.218.175.174
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=linux.dev; s=key1;
	...
	bh=eLTFOoafi0GJYkWU6Rlyr2mCweEmEadpOTxcbs+JWnk=;
	b=Id6sA/GMQZ7FdYZXDPS9290xt3/Y7ercFjDKqBy0eJzmjj+DPgmR5V/rKWvLqPHexANL62
	MCDdViUhgiM7iHn/qgxd/WSlbp5kaz+1DZ1n3p25QwzwCLT//LbSXu826SbN1ehXj15FDp
	fw5YoPUEu3LEYSdC8hirspYPOutoTKQ=
Precedence: bulk
...
In-Reply-To: <c2ab8854-66f9-4b26-bcbc-c138918f0e91@redhat.com>
Date: Wed, 10 Apr 2024 16:37:19 +0900
...
Message-Id: <B6FCB750-9DE1-478A-97CD-9310A4A1A9F0@linux.dev>
...
>>> Hi David,
>>>=20
>>>> On Apr 9, 2024, at 23:45, David Hildenbrand <david@redhat.com> =
wrote:
>>>>=20
...
>>>>>>>>>>>=20
>>>>>>>>>>>=20
>>>>>>>>>>>> On Apr 9, 2024, at 19:04, Ryan Roberts =
...
significant
>>>>>>>>>>>>>>>>>> proportion of
>>>>>>>>>>>>>>>>>> the total boot time, especially when rodata=3Dfull. =
And most of the
>>>>>>>>>>>>>>>>>> time is spent
...
>>>>>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>>>>>> The below shows the execution time of map_mem() =
across a couple of
>>>>>>>>>>>>>>>>>> different
...
=3D
>>>>>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>>>>>>      - Added Tested-by tags (thanks to Eric and =
Itaru)
>>>>>>>>>>>>>>>>>>      - Renamed ___set_pte() -> __set_pte_nosync() =
(per Ard)
>>>>>>>>>>>>>>>>>>      - Reordered patches (biggest impact & least =
controversial first)
>>>>>>>>>>>>>>>>>>      - Reordered alloc/map/unmap functions in mmu.c =
to aid reader
...
>>>>>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>>>>>> Ryan Roberts (4):
>>>>>>>>>>>>>>>>>>      arm64: mm: Don't remap pgtables =
per-cont(pte|pmd) block
...
>>>>>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>>>>> I've build and boot tested the v2 on FVP, base is =
taken from your
>>>>>>>>>>>>>>>>> linux-rr repo. Running run_vmtests.sh on v2 left some =
gup longterm not
>>>>>>>>>>>>>>>>> oks, would you take a look at it? The mm ksefltests =
used is from your
>>>>>>>>>>>>>>>>> linux-rr repo too.
>>>>>>>>>>>>>>>>=20
...
>>>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>>>> Running on VM with 12G memory, split across 2 =
(emulated) NUMA nodes
>>>>>>>>>>>>>>>> (needed by
...
>>>>>>>>>>>>>>>> "
>>>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>>>> Ubuntu userspace running off XFS rootfs. Build and run =
mm selftests
>>>>>>>>>>>>>>>> from same
>>>>>>>>>>>>>>>> git tree.
>>>>>>>>>>>>>>>>=20
...
>>>>>>>>>>>>>>>> gup_longterm.
>>>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>>>> Looks like your errors are all "ftruncate() failed". =
I've seen this
>>>>>>>>>>>>>>>> problem on
>>>>>>>>>>>>>>>> our CI system. There it is due to running the tests =
from NFS file
>>>>>>>>>>>>>>>> system. What
...
>>>>>>>>>>>>>>> failures. When testing your kernel on FVP, I was =
executing the script
>>>>>>>>>>>>>>> from the FVP's host filesystem using 9p.
>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>> I'm not sure exactly what the root cause is. Perhaps =
there isn't enough
>>>>>>>>>>>>>> space on
...
>>>>>>>>>>>>>>=20
>>>>>>>>>>>>>=20
>>>>>>>>>>>>> Attached is the strace=E2=80=99d gup_longterm executiong =
log on your
...
independent of
>>>>>>>>>>>> that?
>>>>>>>>>>>> I was under the impression that it was filesystem related =
and not something
>>>>>>>>>>>> that
...
program fails.
>>>>>>>>>>> It is indeed 9p filesystem related, as I switched to using =
NFS all the
>>>>>>>>>>> issues are gone.
>>>>>>>>>>=20
...
(Operation not
>>>>>>>>>> supported)
>>>>>>>>>> ftruncate(3, 4096)                      =3D -1 ENOENT (No =
such file or
>>>>>>>>>> directory)
>>>>>>>>>=20
>>>>>>>>> Note: I'm wondering if the unlinkat here is the problem that =
makes
>>>>>>>>> ftruncate() with 9p result in weird errors (e.g., the =
hypervisor
...
>>>>>>>>=20
>>>>>>>> The unlink with 9p most certainly was a known issue in the =
past:
>>>>>>>>=20
...
>>>>>>>> completely resolved?
>>>>>>>=20
>>>>>>> I believe Itaru is running on FVP (Fixed Virtual Platform - =
"fast model" -
>>>>>>> Arm's architecture emulator). So QEMU won't be involved here. =
The FVP emulates
...
Real HW with
>>>>>>> Linux running bare metal using an NFS rootfs. I'm not sure if =
this is related.
...
look reasonable.
>=20
> Yeah, but this will hide BUGs elsewhere. I suspect that in Ryan's NFS =
setup is
> also a BUG lurking somewhere in the NFS implementation. But that's =
just a guess
...
>>> Since you=E2=80=99re an expert on GUP-fast (or fast-GUP?), when you =
update the code, could you print out errno as well like the =
split_huge_page_test.c does
>=20
> While we could, I don't see much value in that for selftests. strace =
log is of much
> more valuable to understand what is actually happening (e.g., fstatfs =
failing), and
...
>=20
> openat(AT_FDCWD, "/tmp", O_RDWR|O_EXCL|O_TMPFILE, 0600) =3D 3
> fcntl(3, F_GETFL)                       =3D 0x424002 (flags =
O_RDWR|O_LARGEFILE|O_TMPFILE)
> fstatfs(3, {f_type=3DTMPFS_MAGIC, f_bsize=3D4096, f_blocks=3D416015, =
f_bfree=3D415997, f_bavail=3D415997, f_files=3D416015, f_ffree=3D416009, =
f_fsid=3D{val=3D[0x8e6b7ce6, 0xe1737440]}, f_namelen=3D255, =
f_frsize=3D4096, f_flags=3DST_VALID|ST_RELATIME}) =3D 0
> ftruncate(3, 4096)                      =3D 0
> fallocate(3, 0, 0, 4096)                =3D 0
>=20
> -> TMPFS/SHMEM, works as expected
>=20
> openat(AT_FDCWD, "gup_longterm.c_tmpfile_WMLTNf", =
O_RDWR|O_CREAT|O_EXCL, 0600) =3D 3
> unlinkat(AT_FDCWD, "gup_longterm.c_tmpfile_WMLTNf", 0) =3D 0
> fstatfs(3, {f_type=3DNFS_SUPER_MAGIC, f_bsize=3D1048576, =
f_blocks=3D112200, f_bfree=3D27954, f_bavail=3D23296, f_files=3D7307264, =
f_ffree=3D4724815, f_fsid=3D{val=3D[0, 0]}, f_namelen=3D255, =
f_frsize=3D1048576, f_flags=3DST_VALID|ST_RELATIME}) =3D 0
> ftruncate(3, 4096)                      =3D 0
> fallocate(3, 0, 0, 4096)                =3D 0
>=20
> -> NFS, works as expected
>=20
> Note that you get all skips (not fails), because your kernel is not =
compiled with CONFIG_GUP_TEST.
>=20
> ok 1 # SKIP gup_test not available

I rebuilt the v6.9-rc3 kernel with that option enabled. This time SKIPs =
are due to =E2=80=9Cneed more free huge pages=E2=80=9D, I=E2=80=99ll =
check even on a limited memory size system preparing enough huge pages =
is possible.

...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 1A9D2813484
	for <lists+linux-kernel@lfdr.de>; Thu, 14 Dec 2023 16:20:40 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1573715AbjLNPU2 (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Thu, 14 Dec 2023 10:20:28 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:33900 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230118AbjLNPU0 (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Thu, 14 Dec 2023 10:20:26 -0500
Received: from mail-ed1-x530.google.com (mail-ed1-x530.google.com [IPv6:2a00:1450:4864:20::530])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 5446B120
        for <linux-kernel@vger.kernel.org>; Thu, 14 Dec 2023 07:20:29 -0800 (PST)
Received: by mail-ed1-x530.google.com with SMTP id 4fb4d7f45d1cf-54c67b0da54so10491194a12.0
        for <linux-kernel@vger.kernel.org>; Thu, 14 Dec 2023 07:20:29 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=semihalf.com; s=google; t=1702567228; x=1703172028; darn=vger.kernel.org;
        h=content-transfer-encoding:cc:to:subject:message-id:date:from
         :in-reply-to:references:mime-version:from:to:cc:subject:date
         :message-id:reply-to;
        bh=SFZYBMgvHgJxXr/9DfcVIe05AZYYh/0++dGb/leJ6wA=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=1e100.net; s=20230601; t=1702567228; x=1703172028;
        h=content-transfer-encoding:cc:to:subject:message-id:date:from
         :in-reply-to:references:mime-version:x-gm-message-state:from:to:cc
         :subject:date:message-id:reply-to;
        bh=SFZYBMgvHgJxXr/9DfcVIe05AZYYh/0++dGb/leJ6wA=;
        b=
X-Gm-Message-State: AOJu0Yzsy/yOLnd1TNf1xRFv257vognJu0dTk9Y3P7SgIjWF6tqmSYaf
        BlbHCWBKOOh9JBWHvoHNAQxm7QDwSmm+VOWzitH0
X-Google-Smtp-Source: AGHT+IFFvJUUhDV6z9D41U/m82dZZjRQKJks2G/LJTFh6squDMCIYX0TQqCYCp5LHTX4LcQJZrft1dLe3RZQOckkVdY=
X-Received: by 2002:a50:cc49:0:b0:54c:a432:4e8 with SMTP id
 n9-20020a50cc49000000b0054ca43204e8mr6253583edi.35.1702567227456; Thu, 14 Dec
 2023 07:20:27 -0800 (PST)
MIME-Version: 1.0
...
In-Reply-To: <CAK8Bye+VQshH-AUKmmwVPZ+peo1Ca5961UuC8UB2ATAa4oHLEA@mail.gmail.com>
...
> >   dyndbg: tweak pr_info format s/trace dest/trace_dest/  # greppability=
 squash
...
> >
> >   echo module,test_dynamic_debug,class,L2,+p > /proc/dynamic_debug/cont=
rol
...
> > [ 1273.075365] dyndbg: module: test_dynamic_debug dyndbg=3D"class,D2_CO=
RE,+T:foo%class,D2_KMS,+T:foo"
> > [ 1273.076307] dyndbg: query 0: <class,D2_CORE,+T:foo> on module: <test=
_dynamic_debug>
...
> > [ 1273.078519] dyndbg: query 1: <class,D2_KMS,+T:foo> on module: <test_=
dynamic_debug>
...
> >   [root@v6 lx]# ddgrep \\btrace\\b      # a better/narrower search-term=
 ?
> >   ...
> >   Default trace destination: foo        # add a '#:' prefix to these li=
nes ?
...
> >   Opened trace instances:               # all values should be on this =
line
> >
...
> >    open drm_core
> >    open drm_mix
> >    open driver_1        # we can separate by modname but not drvr-numbe=
r
> >    open driver_2
...
> >    class DRM_UT_CORE +mf                # traces dont do prefixing (sho=
uld it?)
...
I'm a bit lost here. Help me out please. What do you mean by default behavi=
or ?
...
> >     tail -n9 /proc/dynamic_debug/control | grep -P \\w+ | grep -vP ^dri=
vers/
> > }
...
> >     echo \# PRIVATE_TRACE_TEST - proper lifo cycle - open, enable:named=
 disable:named close
> >
...
> >     ddcmd class,D2_CORE,+mf%class,D2_KMS,+lt%class,D2_ATOMIC,+ml "# add=
 some prefixes"
...
> > [    6.726160] dyndbg: class[0]: module:test_dynamic_debug base:22 len:=
8 ty:3
> > [    6.727052] dyndbg: class[1]: module:test_dynamic_debug base:14 len:=
8 ty:1
> > [    6.728158] dyndbg: class[2]: module:test_dynamic_debug base:10 len:=
3 ty:2
> > [    6.729112] dyndbg: class[3]: module:test_dynamic_debug base:0 len:1=
0 ty:0
> > [    6.729969] dyndbg: module:test_dynamic_debug attached 4 classes
> > [    6.730729] dyndbg:  32 debug prints in module test_dynamic_debug
> > [    6.731494] dyndbg: module: test_dynamic_debug dyndbg=3D"class,D2_CO=
RE,+T"
> > [    6.732350] dyndbg: query 0: <class,D2_CORE,+T> on module: <test_dyn=
amic_debug>
> > [    6.733291] dyndbg: processed 1 queries, with 1 matches, 0 errs
> > [    6.734224] ------------[ cut here ]------------
> > [    6.734811] WARNING: CPU: 1 PID: 472 at lib/dynamic_debug.c:1309 dde=
bug_printk+0xde/0xf0
> > [    6.735814] Modules linked in: test_dynamic_debug(E+) intel_rapl_msr=
(E) crc32_pclmul(E) intel_rapl_common(E) ghash_clmulni_intel(E) crct10dif_p=
clmul(E) crc32c_intel(E) joydev(E) serio_raw(E) pcspkr(E) i2c_piix4(E) [las=
t unloaded: test_dynamic_debug(E)]
> > [    6.738594] CPU: 1 PID: 472 Comm: modprobe Tainted: G        W   E  =
    6.6.0-tf2-virtme-00026-g20d113eb6f9a-dirty #220
> > [    6.740008] Hardware name: QEMU Standard PC (i440FX + PIIX, 1996), B=
IOS rel-1.16.3-0-ga6ed6b701f0a-prebuilt.qemu.org 04/01/2014
> > [    6.741669] RIP: 0010:ddebug_printk+0xde/0xf0
> > [    6.742220] Code: 48 8d 44 24 28 48 89 44 24 20 e8 ed 71 9c ff 48 83=
 c4 58 5b 41 5c 5d c3 48 8d 56 02 48 8d 4c 24 10 31 f6 e8 84 f9 ff ff eb b5=
 <0f> 0b eb a0 66 66 2e 0f 1f 84 00 00 00 00 00 0f 1f 00 41 57 48 89
> > [    6.745072] RSP: 0018:ffffb38140453bd0 EFLAGS: 00010246
> > [    6.745914] RAX: 0000000000000000 RBX: ffffffffc03b97e0 RCX: ffffb38=
140453c50
> > [    6.747002] RDX: ffffb38140453be0 RSI: ffffffffb26f8150 RDI: 0000000=
000000000
> > [    6.747883] RBP: ffffb38140453c38 R08: 0000000000000020 R09: fffffff=
fb3370ce4
> > [    6.748753] R10: 0000000000000001 R11: 0000000000010000 R12: fffffff=
fb26f8150
> > [    6.749586] R13: ffff9b68029bc440 R14: ffff9b6805640800 R15: ffff9b6=
805c9d640
> > [    6.750497] FS:  00007f7cdc453740(0000) GS:ffff9b683ec80000(0000) kn=
lGS:0000000000000000
> > [    6.751537] CS:  0010 DS: 0000 ES: 0000 CR0: 0000000080050033
> > [    6.752310] CR2: 000055ce7a609ae0 CR3: 000000000575a000 CR4: 0000000=
000750ee0
...
> > [    6.765522] Code: ff c3 66 2e 0f 1f 84 00 00 00 00 00 90 f3 0f 1e fa=
...
> > [    6.767763] RSP: 002b:00007fff198e28d8 EFLAGS: 00000246 ORIG_RAX: 00=
00000000000139
> > [    6.768781] RAX: ffffffffffffffda RBX: 000055ce7bb53980 RCX: 00007f7=
cdc56b5ad
> > [    6.769611] RDX: 0000000000000000 RSI: 000055ce7bb72930 RDI: 0000000=
000000006
> > [    6.770598] RBP: 00007fff198e2990 R08: 0000000000000000 R09: 0000000=
000000002
> > [    6.771447] R10: 0000000000000006 R11: 0000000000000246 R12: 000055c=
e7bb72930
> > [    6.772328] R13: 0000000000040000 R14: 000055ce7bb53bc0 R15: 000055c=
e7bb72930
> > [    6.773252]  </TASK>
> > [    6.773532] ---[ end trace 0000000000000000 ]---
> > : 2 matches on =3DT
> > drivers/cpufreq/intel_pstate.c:1912 [intel_pstate]core_get_max_pstate =
=3D_ "max_pstate=3DTAC %x\n"
> > lib/test_dynamic_debug.c:109 [test_dynamic_debug]do_cats =3DT:(null) "D=
2_CORE msg\n" class:D2_CORE
> > did do_prints
> > # tracer: nop
> > #
...
> > [    7.991723] dyndbg: class[0]: module:test_dynamic_debug base:22 len:=
8 ty:3
> > [    7.992488] dyndbg: class[1]: module:test_dynamic_debug base:14 len:=
8 ty:1
> > [    7.993178] dyndbg: class[2]: module:test_dynamic_debug base:10 len:=
3 ty:2
> > [    7.993873] dyndbg: class[3]: module:test_dynamic_debug base:0 len:1=
0 ty:0
...
> > #              | |         |   |||||     |         |
> >   dd-selftest.rc-375     [003] .....     6.876234: tracing_mark_write: =
here comes the WARN
> >   dd-selftest.rc-375     [003] .....     8.168406: tracing_mark_write: =
should be ready
...
it is removed as a part of module removal, I will fix it in the next patchs=
et
...
> > [    8.300750] dyndbg: class[0]: module:test_dynamic_debug base:22 len:=
8 ty:3
> > [    8.301509] dyndbg: class[1]: module:test_dynamic_debug base:14 len:=
8 ty:1
> > [    8.302189] dyndbg: class[2]: module:test_dynamic_debug base:10 len:=
3 ty:2
> > [    8.302876] dyndbg: class[3]: module:test_dynamic_debug base:0 len:1=
0 ty:0
...
> > #           TASK-PID     CPU#  |||||  TIMESTAMP  FUNCTION
> > #              | |         |   |||||     |         |
> >   dd-selftest.rc-375     [001] .....     6.934983: tracing_mark_write: =
here comes the WARN
> >   dd-selftest.rc-375     [003] .....     8.208645: tracing_mark_write: =
test_private_trace about to do_prints
> >   dd-selftest.rc-375     [003] .....     8.539307: tracing_mark_write: =
test_private_trace_2 about to do_prints
> > EOF
> >
...
> >     modprobe test_dynamic_debug dyndbg=3Dclass,D2_CORE,+T:foo%class,D2_=
KMS,+T:foo
> >     # triggers:
...
This is again parsing error in read_T_args. I will fix it in the next patch=
set.
